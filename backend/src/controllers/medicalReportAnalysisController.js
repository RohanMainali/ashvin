import MedicalReportAnalysis from '../models/MedicalReportAnalysis.js';
import { analyzeImageWithLLM } from '../services/aiService.js';

// POST /api/medical-report
export const createMedicalReportAnalysis = async (req, res) => {
  try {
    ...
    let insights = '';
    try {
      aiRes = await analyzeImageWithLLM(imageUrl, 'medical_report', 600, userContext || '');
    ...
    const entry = new MedicalReportAnalysis({
      user: userId,
      imageUrl,
      short_summary,
      analysis,
...
export const getMedicalReportAnalyses = async (req, res) => {
  try {
    const userId = req.userId;
    const entries = await MedicalReportAnalysis.find({ user: userId }).sort({ date: -1 });
    res.json(entries);
  } catch (error) {
    res.status(500).json({ message: 'Server error.', error });
  }
...
export const deleteMedicalReportAnalysis = async (req, res) => {
  try {
    const { id } = req.params;
    const userId = req.userId;
    const deleted = await MedicalReportAnalysis.findOneAndDelete({ _id: id, user: userId });
